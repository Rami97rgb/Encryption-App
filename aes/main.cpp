...
void crypter(unsigned char in[], unsigned char out[], unsigned  char key[])
{
...
void decrypter(unsigned char in[], unsigned char out[], unsigned  char key[])
{
  unsigned char *w = new unsigned char[4 * Nb * (Nr + 1)];
  KeyExpansion(key, w);
  unsigned char **state = new unsigned char *[4];
 ...
 main()
{
  ...
  unsigned char plain1[32] ;
  unsigned char key[48] ;
  unsigned char *in= new unsigned char[16];
  unsigned char *out = new unsigned char[16];
  //read message
  cout<<"message ";
  cin.getline((char*)plain,sizeof(plain));
  cout<<"enter key of size 128, 192 or 256 bits ";
  //read key
  cin.getline((char*)key,sizeof(key));
    Nb = 4;
  switch (8*strlen((char*)key))
  {
  case 128:
    Nk = 4;
    Nr = 10;
  ...
  //encrypt
  if (c=="e")
  {for (int j=0;plain[j]!=NULL;j+=15)
  {
      for (int k=0;k<15;k++)
       plain1[k]=plain[k+j];
      crypter(plain1, out , key);
  for(int i=0;i<16;i++)
    {if(out[i]<16)
        cout<<"0";
        cout<<hex<<int(out[i]);
